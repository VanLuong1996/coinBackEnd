//'use strict';
// generated on 2016-08-04 using generator-cg-gas 3.3.4
...
gulp.task('sass', function () {
    return gulp.src(config.mainScss)
        .pipe($.rubySass({compass: true, style: 'compressed'}))
...
gulp.task('build:dist', ['clean'], function (cb) {
    runSequence(['build', 'copy', 'copy:templates', 'copy:layouts', 'copy:ext', 'copy:font', 'images'], 'html', cb);
});
...
gulp.task('copy:layouts', function () {
    return gulp.src([
        config.base + '/layouts/**/*.html'
    ]).pipe(gulp.dest(config.dist + '/layouts'))
        .pipe($.size({
            title: 'copy layouts'
        }));
});
...
gulp.task('copy:ext', function () {
    return gulp.src([
        config.base + '/shared/ext/**/*.*'
    ]).pipe(gulp.dest(config.dist + '/shared/ext'))
        .pipe($.size({
            title: 'copy ext'
        }));
});
...
gulp.task('copy:font', function () {
    return gulp.src([
        config.base + '/vendor/bootstrap/fonts/*.*'
